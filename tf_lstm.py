...
df = pd.read_csv('Copy of DowJonesComponentsDowIndexComp.csv', header=0, index_col=1)
print(df.head(5))
vals = df.values
encode = LabelEncoder()
...
ledom = Sequential()
ledom.add(LSTM(30, input_shape=(train_x.shape[1], train_x.shape[2])))
ledom.add(Dense(1))
ledom.compile(loss='mae', optimizer='adam')
# Fit network
